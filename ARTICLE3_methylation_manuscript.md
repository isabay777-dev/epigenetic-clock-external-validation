<!-- CO-AUTHOR TBD: Track 2 biology portfolio co-author is undecided. Do not include this placeholder in the submitted manuscript body. -->
...
The source methylation datasets are available through the Gene Expression Omnibus under accessions GSE40279 and GSE87571. The analysis outputs used for this manuscript are stored in `results/`, including `external_validation.json`, `conformal_results.json`, `model_comparison_bootstrap.json`, `shap_results.json`, `shap_importance.csv`, `age_gap_results.json`, `age_gap_no_sex_predictions.csv`, and `metrics.json`. The analysis code is available in the accompanying project repository.

### Code availability

The analysis was generated by `make_clock.py`, with figures generated by `make_figures.py`. The manuscript reports only numbers present in the analysis outputs and `ARTICLE3_methylation_facts.md`. The complete code and processed data are openly available in the project repository (https://github.com/isabay777-dev/epigenetic-clock-external-validation) and archived at Zenodo (https://doi.org/10.5281/zenodo.21202338).
